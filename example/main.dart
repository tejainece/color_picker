...
void _create() {
  ...
  querySelector("#large_picker").nodes.add(largeColorPicker.element);
  ...
  var smallColorPicker = new ColorPicker(128,
      showInfoBox: false, initialColor: new ColorValue.fromRGB(60, 190, 220));
  querySelector("#small_picker").nodes.add(smallColorPicker.element);
  // Listen for color change events and update the UI
  smallColorPicker.colorChangeListener =
      (ColorValue color, num hue, num saturation, num brightness) {
    querySelector("#small_color_value").innerHtml = color.toString();
  };
  ...
  querySelector("#tiny_picker").nodes.add(tinyColorPicker.element);
  // Listen for color change events
  tinyColorPicker.colorChangeListener =
      (ColorValue color, num hue, num saturation, num brightness) {
    querySelector("#tiny_color_value").innerHtml = color.toString();
  };
}
